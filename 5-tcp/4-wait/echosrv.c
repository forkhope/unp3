...
static void sig_chld(int signum)
{
     ...
     *
     * Establishing a signal handler and calling wait() from that handler
     * are insufficient for preventing zombies. The problem is that all
     * five signals are generated before the signal handler is executed,
     * and the signal handler is executed only one time because Unix
