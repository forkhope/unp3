...
static void str_echo(int fd)
{
     ...
     * the client, but the server now expects that line to contain two
     * integers separated by white space, and the server returns the sum
    ...
    for ( ; ; ) {
        ...
        /* We call sscanf() to convert the two arguments from text strings
         * to long integers, and then snprintf() is called to convert the
         * result into a text string.
         * The sscanf() returns the number of input items successfully
         * matched and assigned, which can be fewer than provided for, or
         * even zero in the event of an early matching failure. The value
         * EOF is returned if the end of input is reached before either the
         * first successful conversion or a matching failure occurs.
         */
